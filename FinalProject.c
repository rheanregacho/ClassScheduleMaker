...
void viewSched(subj *x, int num){
	...
	for(i = 0; i < num; i++){
		if(x[i].duration == 1){
			if(x[i].classHour <= 8){
				printf("\n\t     %d         0%d00-0%d00         %s", i+1, x[i].classHour, x[i].endTime, x[i].subjCode);
			} else if(x[i].classHour == 9){
				printf("\n\t     %d         0%d00-%d00         %s", i+1, x[i].classHour, x[i].endTime, x[i].subjCode);
			} else{
				printf("\n\t     %d         %d00-%d00         %s", i+1, x[i].classHour, x[i].endTime, x[i].subjCode);
			}	
		} else if(x[i].duration == 2){
			if(x[i].classHour <= 7){
				printf("\n\t     %d         0%d00-0%d00         %s", i+1, x[i].classHour, x[i].endTime, x[i].subjCode);
			} else if(x[i].classHour == 8){
				printf("\n\t     %d         0%d00-%d00         %s", i+1, x[i].classHour, x[i].endTime, x[i].subjCode);
			} else if(x[i].classHour == 9){
				printf("\n\t     %d         0%d00-%d00         %s", i+1, x[i].classHour, x[i].endTime, x[i].subjCode);
			} else{
...
void addClass(subj *x, int num){
	...
	printf("Class duration (1-3 hours only): ");
	scanf("%d", &x[num].duration);
	
	//checks if the given class duration is allowed
	while ((x[num].duration <= 0)||(x[num].duration >= 4)){
		printf("\nClass duration is not allowed.");
		printf("\nRe-enter class duration: ");
		scanf("%d", &x[num].duration);
		printf("\n");
	}
	
	//creates the time of class
	x[num].endTime = x[num].classHour + x[num].duration;
	
	//checks if the end time exceeds the class hours
	while(x[num].endTime>=22){
		printf("\nIt exceeds the class hours.");
		printf("\nRe-enter class duration: ");
		scanf("%d", &x[num].duration);
		printf("\n");
		x[num].endTime = x[num].classHour + x[num].duration;
	...
	//checks if time class overlaps with other classes
	for(i = 0; i < num; i++){
		for(j = 0; j < num; j++){
			if((x[i].classHour < x[j].classHour)||(x[i].endTime > x[j].endTime)){
			printf("\nTime overlaps with another class.");
			printf("\nRe-enter time slot: ");
			scanf("%d", &x[num].classHour);
			printf("Re-enter class duration: ");
			scanf("%d", &x[num].duration);
			printf("\n");
			x[num].endTime = x[num].classHour + x[num].duration;
			}
